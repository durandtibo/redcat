...
To make the package as slim as possible, only the packages required to use `redcat` are installed.
...
To install `redcat` from source, you can follow the steps below. First, you will need to
...
configurations. Then, you should install the required package to use `redcat` with the following
command:
