...
class BatchedArray(np.ndarray):
    ...
    def nancumprod(
        ...
        r"""Return the cumulative product of array elements over a
        ...
        Returns:
            The cumulative product of elements along a specified axis,
        ...
        ```
        """
        x = np.nancumprod(self, axis=axis, dtype=dtype, out=out)
        if out is None and axis is None:
            x = x.view(np.ndarray)
        return x

    def nancumprod_along_batch(
        self, dtype: DTypeLike = None, out: np.ndarray | None = None
    ) -> TBatchedArray:
        r"""Return the cumulative product of array elements over a given
        ...
        ```
        """
        return self.nancumprod(axis=self.batch_axis, dtype=dtype, out=out)
    ...
    def nanargmax(self, *args: Any, **kwargs: Any) -> np.ndarray:
        r"""Return the indices of the maximum values in the specified
        axis ignoring NaNs.
        ...
        >>> from redcat.ba import BatchedArray
        >>> batch = BatchedArray(np.array([[1, np.nan, 2], [3, 4, 5]]))
        >>> batch.nanargmax(axis=0)
        array([1, 1, 1])
        >>> batch.nanargmax(axis=0, keepdims=True)
        array([[1, 1, 1]])
        >>> batch = BatchedArray(np.array([[1, np.nan, 2], [3, 4, 5]]), batch_axis=1)
        >>> batch.nanargmax(axis=1)
        array([2, 2])

        ```
        """
        return np.nanargmax(self.get_ndarray(), *args, **kwargs)

    def nanargmax_along_batch(self, *args: Any, **kwargs: Any) -> np.ndarray:
    ...
    def nanmax(self, *args: Any, **kwargs: Any) -> np.ndarray:
        ...
        ```
        """
        return np.nanmax(self.get_ndarray(), *args, **kwargs)

    def nanmax_along_batch(self, *args: Any, **kwargs: Any) -> np.ndarray:
        r"""Compute the maximum or the maximum along the batch axis,
        ignoring NaNs.
        ...
        ```
        """
        return self.nanmax(*args, axis=self.batch_axis, **kwargs)
