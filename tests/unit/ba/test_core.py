...
def test_batched_array__truediv__(other: np.ndarray | float) -> None:
...
def test_batched_array__itruediv__(other: np.ndarray | float) -> None:
    batch = ba.ones(shape=(2, 3))
    batch /= other
...
def test_batched_array_add_(other: np.ndarray | float) -> None:
    batch = ba.ones(shape=(2, 3))
    batch.add_(other)
...
def test_batched_array_floordiv(other: np.ndarray | float) -> None:
...
def test_batched_array_floordiv_(other: np.ndarray | float) -> None:
    batch = ba.ones((2, 3))
...
def test_batched_array_fmod(other: np.ndarray | float) -> None:
...
def test_batched_array_fmod_(other: np.ndarray | float) -> None:
    batch = ba.ones((2, 3))
    batch.fmod_(other)
    assert batch.allequal(ba.ones(shape=(2, 3)))
...
def test_batched_array_mul(other: np.ndarray | float) -> None:
...
def test_batched_array_mul_(other: np.ndarray | float) -> None:
    batch = ba.ones((2, 3))
    batch.mul_(other)
    assert batch.allequal(ba.full(shape=(2, 3), fill_value=2.0))
...
def test_batched_array_sub(other: np.ndarray | float) -> None:
...
def test_batched_array_sub_(other: np.ndarray | float) -> None:
    batch = ba.ones((2, 3))
    batch.sub_(other)
    assert batch.allequal(ba.full(shape=(2, 3), fill_value=-1.0))
...
def test_batched_array_truediv(other: np.ndarray | float) -> None:
...
def test_batched_array_truediv_(other: np.ndarray | float) -> None:
    batch = ba.ones((2, 3))
...
def test_batched_array_min_1d() -> None:
    assert objects_are_equal(
        ba.BatchedArray(np.array([4, 1, 2, 5, 3])).min(axis=0),
        np.int64(1),
    )
...
def test_batched_array_min_2d(dtype: np.dtype) -> None:
    assert objects_are_equal(
        ba.BatchedArray(np.array([[4, 9], [1, 7], [2, 5], [5, 6], [3, 8]], dtype=dtype)).min(
            axis=0
        ),
...
def test_batched_array_min_axis_none() -> None:
    assert objects_are_equal(
        ba.BatchedArray(np.array([[4, 9], [1, 7], [2, 5], [5, 6], [3, 8]])).min(axis=None),
        np.int64(1),
    )
...
def test_batched_array_min_out_axis_none() -> None:
    out = np.array(0, dtype=int)
    assert ba.BatchedArray(np.array([[4, 9], [1, 7], [2, 5], [5, 6], [3, 8]])).min(out=out) is out
...
def test_batched_array_min_out_axis_0() -> None:
    out = np.array([0, 0], dtype=int)
    assert (
        ba.BatchedArray(np.array([[4, 9], [1, 7], [2, 5], [5, 6], [3, 8]])).min(axis=0, out=out)
        is out
    )
...
def test_batched_array_min_custom_axes() -> None:
    assert objects_are_equal(
        ba.BatchedArray(np.array([[4, 1, 2, 5, 3], [9, 7, 5, 6, 8]]), batch_axis=1).min(axis=1),
        np.array([1, 5]),
    )
...
def test_batched_array_min_along_batch_out() -> None:
    out = np.array([0, 0], dtype=int)
    assert (
        ba.BatchedArray(np.array([[4, 9], [1, 7], [2, 5], [5, 6], [3, 8]])).min_along_batch(out=out)
        is out
    )
    assert objects_are_equal(out, np.array([1, 5], dtype=int))
...
def test_batched_array_min_along_batch_keepdims() -> None:
    assert objects_are_equal(
        ba.BatchedArray(np.array([[4, 9], [1, 7], [2, 5], [5, 6], [3, 8]])).min_along_batch(
            keepdims=True
        ),
        np.array([[1, 5]]),
    )
...
def test_batched_array_nanmax_1d() -> None:
    assert objects_are_equal(
        ba.BatchedArray(np.array([4, 1, 2, np.nan, 3])).nanmax(axis=0),
        np.float64(4.0),
    )
...
def test_batched_array_nanmax_2d() -> None:
    assert objects_are_equal(
        ba.BatchedArray(np.array([[4, np.nan], [1, 7], [np.nan, 5], [5, 6], [3, 8]])).nanmax(
            axis=0
        ),
        np.array([5.0, 8.0]),
    )
...
def test_batched_array_nanmax_custom_axes() -> None:
    assert objects_are_equal(
        ba.BatchedArray(
            np.array([[4, 1, np.nan, 5, 3], [np.nan, 7, 5, 6, 8]]), batch_axis=1
        ).nanmax(axis=1),
        np.array([5.0, 8.0]),
    )
...
def test_batched_array_nanmax_along_batch() -> None:
    assert objects_are_equal(
        ba.BatchedArray(
            np.array([[4, np.nan], [1, 7], [np.nan, 5], [5, 6], [3, 8]])
        ).nanmax_along_batch(),
        np.array([5.0, 8.0]),
    )
...
def test_batched_array_nanmax_along_batch_keepdims() -> None:
    assert objects_are_equal(
        ba.BatchedArray(
            np.array([[4, np.nan], [1, 7], [np.nan, 5], [5, 6], [3, 8]])
        ).nanmax_along_batch(keepdims=True),
        np.array([[5.0, 8.0]]),
    )
...
def test_batched_array_nanmin_1d() -> None:
    assert objects_are_equal(
        ba.BatchedArray(np.array([4, np.nan, 2, 5, 3])).nanmin(axis=0),
        np.float64(2.0),
    )
...
def test_batched_array_nanmin_2d() -> None:
    assert objects_are_equal(
        ba.BatchedArray(np.array([[4, np.nan], [np.nan, 7], [2, 5], [5, 6], [3, 8]])).nanmin(
            axis=0
        ),
        np.array([2.0, 5.0]),
    )
...
def test_batched_array_nanmin_out_axis_none() -> None:
    out = np.array(0.0)
    assert (
        ba.BatchedArray(np.array([[4, np.nan], [np.nan, 7], [2, 5], [5, 6], [3, 8]])).nanmin(
            out=out
    ...
    assert objects_are_equal(out, np.array(2.0))


def test_batched_array_nanmin_out_axis_0() -> None:
    out = np.array([0.0, 0.0])
    assert (
        ba.BatchedArray(np.array([[4, np.nan], [np.nan, 7], [2, 5], [5, 6], [3, 8]])).nanmin(
            axis=0, out=out
    ...
    assert objects_are_equal(out, np.array([2.0, 5.0]))


def test_batched_array_nanmin_custom_axes() -> None:
    assert objects_are_equal(
        ba.BatchedArray(
            np.array([[4, np.nan, 2, 5, 3], [np.nan, 7, 5, 6, 8]]), batch_axis=1
        ).nanmin(axis=1),
        np.array([2.0, 5.0]),
    )
...
def test_batched_array_nanmin_along_batch() -> None:
    assert objects_are_equal(
        ba.BatchedArray(
            np.array([[4, np.nan], [np.nan, 7], [2, 5], [5, 6], [3, 8]])
        ).nanmin_along_batch(),
        np.array([2.0, 5.0]),
    )


def test_batched_array_nanmin_along_batch_out() -> None:
    out = np.array([0.0, 0.0])
    assert (
        ba.BatchedArray(
            np.array([[4, np.nan], [np.nan, 7], [2, 5], [5, 6], [3, 8]])
        ).nanmin_along_batch(out=out)
        is out
    )
    assert objects_are_equal(out, np.array([2.0, 5.0], dtype=float))
...
def test_batched_array_nanmin_along_batch_keepdims() -> None:
    assert objects_are_equal(
        ba.BatchedArray(
            np.array([[4, np.nan], [np.nan, 7], [2, 5], [5, 6], [3, 8]])
        ).nanmin_along_batch(keepdims=True),
        np.array([[2.0, 5.0]]),
    )
...
def test_batched_array_mean_1d() -> None:
    assert objects_are_equal(
        ba.BatchedArray(np.array([4, 1, 2, 5, 3])).mean(axis=0),
        np.float64(3.0),
    )
...
def test_batched_array_mean_2d(dtype: np.dtype) -> None:
    assert objects_are_equal(
        ba.BatchedArray(np.array([[4, 9], [1, 7], [2, 5], [5, 6], [3, 8]], dtype=dtype)).mean(
            axis=0
        ),
        np.array([3.0, 7.0]),
    )
...
def test_batched_array_mean_axis_none() -> None:
    assert objects_are_equal(
        ba.BatchedArray(np.array([[4, 9], [1, 7], [2, 5], [5, 6], [3, 8]])).mean(axis=None),
        np.float64(5.0),
    )
...
def test_batched_array_mean_out_axis_none() -> None:
    out = np.array(0.0)
    assert ba.BatchedArray(np.array([[4, 9], [1, 7], [2, 5], [5, 6], [3, 8]])).mean(out=out) is out
...
def test_batched_array_mean_out_axis_0() -> None:
    out = np.array([0.0, 0.0])
    assert (
        ba.BatchedArray(np.array([[4, 9], [1, 7], [2, 5], [5, 6], [3, 8]])).mean(axis=0, out=out)
        is out
    )
...
def test_batched_array_mean_custom_axes() -> None:
    assert objects_are_equal(
        ba.BatchedArray(np.array([[4, 1, 2, 5, 3], [9, 7, 5, 6, 8]]), batch_axis=1).mean(axis=1),
        np.array([3.0, 7.0]),
    )
...
def test_batched_array_mean_along_batch_keepdims() -> None:
    assert objects_are_equal(
        ba.BatchedArray(np.array([[4, 9], [1, 7], [2, 5], [5, 6], [3, 8]])).mean_along_batch(
            keepdims=True
        ),
        np.array([[3.0, 7.0]]),
    )
...
def test_batched_array_nanmean_1d() -> None:
    assert objects_are_equal(
        ba.BatchedArray(np.array([4, 1, 2, 5, np.nan])).nanmean(axis=0),
        np.float64(3.0),
    )
...
def test_batched_array_nanmean_2d() -> None:
    assert objects_are_equal(
        ba.BatchedArray(np.array([[1, np.nan, 2], [3, 4, 5]])).nanmean(axis=0),
...
def test_batched_array_nanmean_axis_none() -> None:
    assert objects_are_equal(
        ba.BatchedArray(np.array([[1, np.nan, 2], [3, 4, 5]])).nanmean(axis=None),
        np.float64(3.0),
    )
...
def test_batched_array_nanmean_out_axis_none() -> None:
    out = np.array(0.0)
    assert ba.BatchedArray(np.array([[1, np.nan, 2], [3, 4, 5]])).nanmean(out=out) is out
    assert objects_are_equal(out, np.array(3.0))


def test_batched_array_nanmean_out_axis_0() -> None:
    out = np.array([0.0, 0.0, 0.0])
    assert ba.BatchedArray(np.array([[1, np.nan, 2], [3, 4, 5]])).nanmean(axis=0, out=out) is out
    assert objects_are_equal(out, np.array([2.0, 4.0, 3.5]))


def test_batched_array_nanmean_custom_axes() -> None:
    assert objects_are_equal(
        ba.BatchedArray(np.array([[1, np.nan, 2], [3, 4, 5]]), batch_axis=1).nanmean(axis=1),
        np.array([1.5, 4.0]),
    )
...
def test_batched_array_nanmean_along_batch() -> None:
    assert objects_are_equal(
        ba.BatchedArray(np.array([[1, np.nan, 2], [3, 4, 5]])).nanmean_along_batch(),
        np.array([2.0, 4.0, 3.5]),
    )
...
def test_batched_array_nanmean_along_batch_keepdims() -> None:
    assert objects_are_equal(
        ba.BatchedArray(np.array([[1, np.nan, 2], [3, 4, 5]])).nanmean_along_batch(keepdims=True),
        np.array([[2.0, 4.0, 3.5]]),
    )
...
def test_batched_array_nanmean_along_batch_out() -> None:
    out = np.array([0.0, 0.0, 0.0])
    assert (
        ba.BatchedArray(np.array([[1, np.nan, 2], [3, 4, 5]])).nanmean_along_batch(out=out) is out
    )
    assert objects_are_equal(out, np.array([2.0, 4.0, 3.5]))
...
def test_batched_array_nanmean_along_batch_custom_axes() -> None:
    assert objects_are_equal(
        ba.BatchedArray(np.array([[1, np.nan, 2], [3, 4, 5]]), batch_axis=1).nanmean_along_batch(),
        np.array([1.5, 4.0]),
    )
...
def test_batched_array_nanmedian_1d() -> None:
    assert objects_are_equal(
        ba.BatchedArray(np.array([4, 1, 2, 5, np.nan])).nanmedian(axis=0),
        np.float64(3.0),
    )
...
def test_batched_array_nanmedian_2d() -> None:
    assert objects_are_equal(
        ba.BatchedArray(np.array([[1, np.nan, 2], [3, 4, 5]])).nanmedian(axis=0),
        np.array([2.0, 4.0, 3.5]),
    )
...
def test_batched_array_nanmedian_axis_none() -> None:
    assert objects_are_equal(
        ba.BatchedArray(np.array([[1, np.nan, 2], [3, 4, 5]])).nanmedian(axis=None),
        np.float64(3.0),
    )


def test_batched_array_nanmedian_out_axis_none() -> None:
    out = np.array(0.0)
    assert ba.BatchedArray(np.array([[1, np.nan, 2], [3, 4, 5]])).nanmedian(out=out) is out
    assert objects_are_equal(out, np.array(3.0))


def test_batched_array_nanmedian_out_axis_0() -> None:
    out = np.array([0.0, 0.0, 0.0])
    assert ba.BatchedArray(np.array([[1, np.nan, 2], [3, 4, 5]])).nanmedian(axis=0, out=out) is out
    assert objects_are_equal(out, np.array([2.0, 4.0, 3.5]))


def test_batched_array_nanmedian_custom_axes() -> None:
    assert objects_are_equal(
        ba.BatchedArray(np.array([[1, np.nan, 2], [3, 4, 5]]), batch_axis=1).nanmedian(axis=1),
        np.array([1.5, 4.0]),
    )
...
def test_batched_array_nanmedian_along_batch() -> None:
    assert objects_are_equal(
        ba.BatchedArray(np.array([[1, np.nan, 2], [3, 4, 5]])).nanmedian_along_batch(),
        np.array([2.0, 4.0, 3.5]),
    )
...
def test_batched_array_nanmedian_along_batch_keepdims() -> None:
    assert objects_are_equal(
        ba.BatchedArray(np.array([[1, np.nan, 2], [3, 4, 5]])).nanmedian_along_batch(keepdims=True),
        np.array([[2.0, 4.0, 3.5]]),
    )


def test_batched_array_nanmedian_along_batch_out() -> None:
    out = np.array([0.0, 0.0, 0.0])
    assert (
        ba.BatchedArray(np.array([[1, np.nan, 2], [3, 4, 5]])).nanmedian_along_batch(out=out) is out
    )
    assert objects_are_equal(out, np.array([2.0, 4.0, 3.5]))
