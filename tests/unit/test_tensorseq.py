...
def test_batched_tensor_seq_sort_dim_0() -> None:
    assert objects_are_equal(
        BatchedTensorSeq(torch.tensor([[4, 9], [1, 7], [2, 5], [5, 6], [3, 8]])).sort(dim=0),
        torch.return_types.sort(
            [
...
def test_batched_tensor_seq_sort_along_batch_descending_false() -> None:
    assert objects_are_equal(
        BatchedTensorSeq(torch.tensor([[4, 9], [1, 7], [2, 5], [5, 6], [3, 8]])).sort_along_batch(),
        torch.return_types.sort(
            [
                BatchedTensorSeq(torch.tensor([[1, 5], [2, 6], [3, 7], [4, 8], [5, 9]])),
                BatchedTensorSeq(torch.tensor([[1, 2], [2, 3], [4, 1], [0, 4], [3, 0]])),
...
def test_batched_tensor_seq_maximum_incorrect_batch_dim() -> None:
    batch = BatchedTensorSeq(torch.ones(2, 3, 1))
    with raises(RuntimeError, match=r"The batch dimensions do not match."):
        batch.maximum(BatchedTensorSeq(torch.ones(2, 3, 1), batch_dim=2))


def test_batched_tensor_seq_maximum_incorrect_seq_dim() -> None:
    batch = BatchedTensorSeq(torch.ones(2, 3, 1))
    with raises(RuntimeError, match=r"The sequence dimensions do not match."):
        batch.maximum(BatchedTensorSeq(torch.zeros(2, 1, 3), seq_dim=2))
...
def test_batched_tensor_seq_minimum_incorrect_batch_dim() -> None:
    batch = BatchedTensorSeq(torch.ones(2, 3, 1))
    with raises(RuntimeError, match=r"The batch dimensions do not match."):
        batch.minimum(BatchedTensorSeq(torch.ones(2, 3, 1), batch_dim=2))


def test_batched_tensor_seq_minimum_incorrect_seq_dim() -> None:
    batch = BatchedTensorSeq(torch.ones(2, 3, 1))
    with raises(RuntimeError, match=r"The sequence dimensions do not match."):
        batch.minimum(BatchedTensorSeq(torch.zeros(2, 1, 3), seq_dim=2))
...
def test_batched_tensor_seq_max_along_batch() -> None:
    assert objects_are_equal(
        BatchedTensorSeq(torch.tensor([[0, 5], [1, 6], [2, 7], [3, 8], [4, 9]])).max_along_batch(),
        torch.return_types.max([torch.tensor([4, 9]), torch.tensor([4, 4])]),
    )
...
def test_batched_tensor_seq_min_along_seq_custom_dims() -> None:
    assert objects_are_equal(
        BatchedTensorSeq(torch.tensor([[0, 5], [1, 6], [2, 7], [3, 8], [4, 9]]), batch_dim=1, seq_dim=0).min_along_seq(),
        torch.return_types.min([torch.tensor([0, 5]), torch.tensor([0, 0])]),
    )
...
def test_torch_min_dim_1() -> None:
    assert objects_are_equal(
        torch.min(BatchedTensorSeq(torch.arange(10).view(2, 5)), dim=1),
        torch.return_types.min([torch.tensor([0, 5]), torch.tensor([0, 0])]),
    )
...
def test_torch_min_dim_1_keepdim() -> None:
    assert objects_are_equal(
        torch.min(BatchedTensorSeq(torch.arange(10).view(2, 5)), dim=1, keepdim=True),
        torch.return_types.min([torch.tensor([[0], [5]]), torch.tensor([[0], [0]])]),
    )
